              ...
              ifneq ($(MAKECMDGOALS),distribute)
                ifneq ($(MAKECMDGOALS),info)
                  ifneq ($(MAKECMDGOALS),depends)
                    ifeq ($(BOARD_PORT),ssh)
                        YUN_ADDRESS = $(shell arp -n Arduino.local | sed 's/.*(//' | sed 's/).*//')
                        BACK_ADDRESS = $(shell ifconfig | grep "inet " | grep -v 127.0.0.1 | cut -d\  -f2)
                        ifeq ($(YUN_ADDRESS),)
                            $(error Arduino.local not available)
                        endif
                        ifneq ($(findstring 'Unknown',$(YUN_ADDRESS)),)
                            $(error Arduino.local not available)
                        endif
...
CPPFLAGS     += $(SYS_INCLUDES) -g -Os $(WARNING_FLAGS) -ffunction-sections -fdata-sections
...
ifeq ($(AVRDUDE_NO_SERIAL_PORT),1)
		...
		@echo . Local libraries from $(LOCAL_LIB_PATH)
